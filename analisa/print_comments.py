...
for num in [1, 2, 3]:
    req = urllib.request.Request(f'https://api.github.com/repos/zeroide0/prsolve/issues/{num}/comments', headers=headers)
    with urllib.request.urlopen(req) as resp:
        comments = json.loads(resp.read().decode())
    print(f'=== Issue #{num} ({len(comments)} comments) ===')
    for c in comments:
        print(f"[{c['user']['login']}] {c['created_at']}:")
        clean = c['body'][:300].encode('ascii', errors='replace').decode('ascii')
        print(clean)
        print('-'*40)
